...
class OrdinalCrossEntropyLoss(nn.Module):
    ...
    def forward(self, logits, targets):
        ...
        mask = torch.arange(num_classes).unsqueeze(0).expand(batch_size, num_classes).to(targets.device)
        binary_labels = (mask >= targets).float()

        # Compute binary cross-entropy loss for each class
